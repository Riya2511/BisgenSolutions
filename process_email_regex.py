...
def extract_name(text):
    ...
    if match:
        return (
            match.group(1)
            .strip()
            .title()[: match.group(1).strip().title().index("\\R")]
        )
    return None
...
def extract_qualification(text):
    ...
    if match:
        return (
            match.group(1)
            .strip()
            .upper()[: match.group(1).strip().upper().index("\\R")]
        )
    ...
    for pattern, column_mapping in zip(regex_patterns, column_mappings):
        try:
            ...
            if "instant" in column_mapping.lower():
                ...
                if len(matches) == 0:
                    temp_results.append(
                        (pattern, column_mapping, None, "PARSING_ERROR")
                    )
                ...
                for match in matches:
                    try:
                        ...
                        parsing_status = (
                            "PARSING_DONE" if regex_output else "PARSING_ERROR"
                        )
                        temp_results.append(
                            (pattern, column_mapping, regex_output, parsing_status)
                        )

                        logger.debug(
                            f"Instant Pattern: {pattern}, Column: {column_mapping}, "
                            f"Output: {regex_output}, Status: {parsing_status}"
                        )

                    except Exception as e:
                        logger.error(
                            f"Error processing instant match for pattern {pattern}: {str(e)}"
                        )
                        temp_results.append(
                            (pattern, column_mapping, None, "PARSING_ERROR")
                        )
                ...
                logger.debug(
                    f"Pattern: {pattern}, Column: {column_mapping}, "
                    f"Output: {regex_output}, Status: {parsing_status}"
                )
...
def update_email_regex_results(
    ...
    try:
        ...
        for pattern, column_mapping, output, status in regex_results:
            ...
            regex_applications.append({"regex": pattern, "status": status})
            if output and column_mapping:
                if "name_buyer" in column_mapping:
                    ...
                    lead_updates["requirement"], lead_updates["mail_buyer_location"] = (
                        parse_location_instant(output)
                    )
                ...
                elif "name_aster" in column_mapping:
                    lead_updates["name"] = extract_name(output)
                elif "email_aster" in column_mapping:
                    lead_updates["email"] = extract_email(output)
                elif "phone_aster" in column_mapping:
                    lead_updates["phone"] = extract_phone(output)
                elif "qualification_aster" in column_mapping:
        ...
        cursor.execute(
            "UPDATE email SET status = 'FAIL_TO_PROCESS' WHERE id = %s", (email_id,)
        )
        conn.commit()
    finally:
        cursor.close()
...
def main():
    ...
    for email in emails:
        ...
        regex_results = process_email_regex(
            email_id, imap_body, regex_patterns, column_mappings
        )
        ...
        for regex_result in regex_results:
            success_count = sum(
                1 for _, _, output, _ in regex_result if output is not None
            )
            total_patterns = len(regex_patterns)
            logger.info(
                f"Processed email ID {email_id}: {success_count}/{total_patterns} patterns successful"
            )
